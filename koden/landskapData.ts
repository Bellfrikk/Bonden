...
type LandskapType = 'eng'|'vatn'|'skog'|'grus'|'asfalt'|'jorde'|'veiSving'|'veiXkryss'|'veiTkryss'|'veiBein';
...
type OrginalLandskap = { [key in LandskapType]: { utsnitt: { x: number; y: number; }[]; krasj: null | { b: number; f: number; v: number; h: number; }, retning:number[], fart: number; arbeid: null | { aktivertAv: null | string[]; }, butikk:ButikkInfo ; };};
