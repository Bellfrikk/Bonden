...
type Knappar = {
  ...
  ikonStr: number;
  etasjer: number;
  aktivListe: KnapparTypar[];
  spelListe: KnapparTypar[];
  lagVerdenListe: KnapparTypar[];
  fram: KnappMal;
  bak: KnappMal;
  utAvDoning: KnappMal;
...
type KnapparTypar = "fram"|"bak"|"utAvDoning"|"velgFro"|"aktiverRedskapFram"|"koblingRedskapFram"|"koblingRedskapBak"|"aktiverRedskapBak"|"venstre"|"hogre"|"eng"|"grus"|"jorde"|"veiBeint"|"veiSving"|"veiTkryss"|"veiXkryss"|"vatn";
  ...
  function sjekkKnappar(mus:any, trykk:boolean) {
    ...
    if (aktivSkjerm.verden) {
      for (let k = 0; k < knappar.aktivListe.length; k++) {
        if (knappar[knappar.aktivListe[k]].vis === false) { continue; }//ikkje sjekk knappar som ikkje er aktive
