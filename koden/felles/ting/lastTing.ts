class Last extends TingMal {
  grafikk : Record<string, { skalVise:boolean, tegneRekkefolge: 'under'|'over', midt: { x: 0.5, y: 0.5 }, pos: { x: number, y: number }, str: { x: number, y: number }, retning: number, klippPos: { x: number, y: number } } >;
  constructor(ny:LastData, rute:Posisjon) {
    super(ny, rute);
    this.grafikk = ny.grafikk;
  }
}

interface LastData extends TingDataMal {
  grafikk: Record<string, {  skalVise:boolean, tegneRekkefolge: 'under'|'over', midt: { x: 0.5, y: 0.5 }, pos: { x: number, y: number }, str: { x: number, y: number }, retning: number, klippPos: { x: number, y: number } } >;
}
...
const fro:LastData = {
  ...
  funksjonane:{
    lastAnimasjon: (denne: any) => {
      if(denne.type !== 'last') return;
