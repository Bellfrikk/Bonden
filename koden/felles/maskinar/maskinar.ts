...
//====================================================== flytting av doning ======================================================================
function lagFlyttePosTilDoning(framBak:'fram'|'bak') {
...
//====================================================== nyRetningDoning ======================================================================
function nyRetningDoning(sving:'venstre'|'hogre'|'beint') {
  if (krasjITilhengerTest() === "krasj") { return; } // ikkje sving om du krasje i tilhengaren
...
function teinAlleMaskinar() {
  ...
  let tegneliste:[Maskin,GrafikkDelBase][] = [];
  for ( let denneMaskinNavn in maskinar) {
    let denneMaskin = maskinar[denneMaskinNavn];
    for ( let denneDelNavn in denneMaskin.grafikk) {
      let denneDel:GrafikkDelBase = denneMaskin.grafikk[denneDelNavn as keyof typeof denneMaskin.grafikk];
...
function nyPosisjonDoningOgRedskap() {
  ...
  function posRedskap(denneRedskap:Redskap|null, denneDoningKrok:PosisjonMal) {
    if (denneRedskap === null) { return }
...
function krasjtestDoning(denne:Maskin, framBak:'fram'|'bak'):['ok']|['krasj',Maskin|Ting] {
  for (let t = 0; t < denne.rute.tilSjekk.length; t++) {
    ...
    if (
      ...
      if (doning.type === 'bonde' && koblingskarantene('sjekk', tmpTing as Maskin)){ return(['ok']); }// ikkje sjekk mot ting i karantene viss du er bonde

      if(     framBak === 'fram' && denne.redskap.fram === null && krasjTest( denne.krasj.framSider,              tmpTing.krasj )[0] === 'krasj') { return ['krasj',tmpTing];}
      else if(framBak === 'fram' && denne.redskap.fram !== null && krasjTest( denne.redskap.fram.krasj.framSider, tmpTing.krasj )[0] === 'krasj') { return ['krasj',tmpTing];}
      else if(framBak === 'bak'  && denne.redskap.bak  === null && krasjTest( denne.krasj.bakSider,               tmpTing.krasj )[0] === 'krasj') { return ['krasj',tmpTing];}
      else if(framBak === 'bak'  && denne.redskap.bak  !== null && krasjTest( denne.redskap.bak.krasj.bakSider,   tmpTing.krasj )[0] === 'krasj') { return ['krasj',tmpTing];}
    }
  }
  return ["ok"];
}
...
//====================================================== redskapKoblingSjekk ======================================================================
function redskapKoblingSjekk(denne:Redskap) {
  //sjekker om doning kan koblast til redskap
if (koblingskarantene('sjekk', denne) || !["traktor", "skurtreskar"].includes(doning.arbeid.type)) { return; }
  let posD, posR,aktivKrok:'fram' | 'bak';
...
function redskapKobling(framEllerBak:'fram'|'bak') {
...
//====================================================== aktiverRedskap ======================================================================
function aktiverRedskap(framEllerBak:'fram'|'bak') {
  let denne = doning.redskap[framEllerBak];
  //deaktiver
...
function koblingskarantene (funkjson:'sett'|'sjekk'|'fjerning', denne:Maskin) {
  if (funkjson === 'sett'){
    ...
    if (koblingskaranteneTing === null ) { return false;}
