...
class Slamaskin extends MaskinRedskapMal {
  grafikk: {
    karosseri :GrafikkDelStrY
  };

  constructor (ny:SlamaskinData, rute:Posisjon){
    super(ny, rute)
    this.grafikk = {
...
interface SlamaskinData extends MaskinRedskapData {
  grafikk: {
    karosseri :GrafikkDelStrY
  };
}
...
const slamaskin3bak:SlamaskinData = {
  navn: 'Slåmaskin bak',
  type: 'bakFeste',
  pos: {
    dor: { dx: 0, dy: 0},
  ...
  funksjonane: {
    redskapFramAktivert: (denne:any) => {
      if(denne.type !== 'slamaskin') return;
...
const slamaskin3fram:SlamaskinData = {
