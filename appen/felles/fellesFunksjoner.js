"use strict";
function oppdaterFart(hendelse, krasjting) {
    //----justering av fart----
    if (hendelse === "framKnapp") {
        ...
        if (doning.redskap.fram !== null) {
            doning.fart.arbeid += doning.redskap.fram.arbeid.aktiv ? doning.redskap.fram.fart.arbeid : 0;
        }
        if (doning.redskap.bak !== null) {
            doning.fart.arbeid += doning.redskap.bak.arbeid.aktiv ? doning.redskap.bak.fart.arbeid : 0;
        }
    }
 ...
 */
function oppdaterPoisjonar(denne, lagrePlass) {
    //oppdater alle krasjpunkt
    if (lagrePlass === 'direkte') {
        Object.keys(denne.krasj.punkt).forEach(punktet => { oppdaterPos(denne, denne.krasj.punkt[punktet]); });
    }
    else if (lagrePlass === 'tmp') {
        Object.keys(denne.krasj.punkt).forEach(punktet => { oppdaterTmpPos(denne, denne.krasj.punkt[punktet]); });
    }
    // oppdater rute pos
    oppdaterRutePos(denne);
}
function oppdaterPos(denne, punkt) {
...
function oppdaterTmpPos(denne, punkt) {
...
function lagreOppdaterteTingPosisjonar(denne) {
    ...
    if (denne.type === 'ting') {
        return;
    } // ting har ikkje det som er under her
...
function oppdaterRutePos(denne) {
...
function oppdaterRuterTilSjekk(denne) {
    denne.rute.tilSjekk = [];
    ...
    function sjekkRute(tmpTing) {
        if (erInnaforSjekk(doning.rute, tmpTing.rute, 3)) {
            denne.rute.tilSjekk.push(tmpTing);
        }
    }
}
//--------------------------------------------------- krasj i ting test -----------------------------------------
...
function krasjTest(tingASide, tingB) {
    let A, B, C, D;
    for (let side = 0; side < tingASide.length; side++) {
        A = tingASide[side][0];
        B = tingASide[side][1];
        let tmpAlleSider = [];
        if (tingB.framSider.length > 0) {
            tingB.framSider.forEach(sideX => { tmpAlleSider.push(sideX); });
        }
        if (tingB.bakSider.length > 0) {
            tingB.bakSider.forEach(sideX => { tmpAlleSider.push(sideX); });
        }
        if (tingB.andreSider.length > 0) {
            tingB.andreSider.forEach(sideX => { tmpAlleSider.push(sideX); });
        }
        if (tmpAlleSider.length === 0) {
            return ['ok'];
        }
...
function arbeidPaRute() {
    ...
    function sjekkOmRedskapArbeide(denne) {
        denne.arbeid.punkt.forEach(punkt => {
            ...
            if (!sjekkOmRuteOgArbeidStemmer(jordeBit, denne.arbeid.type)) {
                return;
            }
...
function finnRuteSomArbeidspunktJobbePa(denne, punkt) {
...
function sjekkOmRuteOgArbeidStemmer(jordeBit, arbeidstype) {
    if (jordeBit === undefined) {
        return false;
    }
    if (jordeBit.arbeid.aktivertAv.includes(arbeidstype)) {
        return true;
    } //sjekk at redskap er på rett type jorde
    else {
        return false;
    }
}
function lagNyRuteTypeVisstDetErPlassILass(denne, jordeBit, arbeidsType) {
...
function erInnaforSjekk(tingPos, plassPos, margin) {
