"use strict";
const verktoyKnappar = ['veiSving', 'veiXkryss', 'veiTkryss', 'veiBein', 'grus', 'asfalt', 'skog', 'vatn', 'eng', 'jorde'];
let nyttKart = { startPeng: 0, antalRuter: { x: 10, y: 10 }, storrelseRuter: { x: 50, y: 50 }, landskap: {}, ting: [], maskinar: [] };
const rammeKart = document.getElementById('lagKartSkjerm');
const lerretKart = rammeKart.getContext("2d");
const landskapBildeKart = document.getElementById('landskap');
const jordeBildeKart = document.getElementById('jorde');
...
function teinTingEllerMaskin(detteLerret, bilde, denne) {
    ...
    //teiner doning
    detteLerret.drawImage(bilde, orginalLandskap[denne.type].utsnitt[0].x, orginalLandskap[denne.type].utsnitt[0].y, // velg utsnitt av doningtegning 
    denne.str.x, denne.str.y, //bredde og høyde på utsnitt
    denne.str.x * 0.5, denne.str.y * 0.5, //posisjon av tegning
    denne.str.x, denne.str.y);
...
let aktivType = 'eng';
let aktivKnapp = null;
function velgVerktoy(type, knapp) {
    aktivType = type;
    if (aktivKnapp !== null)
        aktivKnapp.className = 'knapp';
    aktivKnapp = knapp;
    aktivKnapp.className = 'knapp knappValgt';
}
function klikkRute(div, denneRute) {
